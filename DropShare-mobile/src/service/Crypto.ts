...
//   static generateAESKey(): { key: string; iv: string } {
//     const key = CryptoJS.lib.WordArray.random(this.AES_KEY_SIZE);
...
//       return result;
//     } catch (error) {
//       Logger.error("Failed to encrypt chunk", error);
//       throw new DropShareError(
//         ERROR_CODES.ENCRYPTION_FAILED,
...
//         }
//       );
//       const result = Buffer.from(decrypted.toString(CryptoJS.enc.Hex), "hex");
//       // Clear intermediate buffers
//       encryptedChunk.fill(0);
//       return result;
//     } catch (error) {
//       Logger.error("Failed to decrypt chunk", error);
//       throw new DropShareError(
//         ERROR_CODES.ENCRYPTION_FAILED,
...
export class Crypto {
  ...
  static generateRSAKeyPair(): { publicKey: string; privateKey: string } {
    try {
      // Simulate RSA key generation (in production, use a native module)
      const publicKey = Buffer.from(
        crypto.getRandomValues(new Uint8Array(this.RSA_KEY_SIZE / 8))
      ).toString("base64");
  ...
  static encryptAESKeyWithRSA(aesKey: string, _rsaPublicKey: string): string {
    try {
      // Simulate RSA encryption (in production, use a proper RSA library)
  ...
  static createEncryptedPayload(
    data: Buffer,
    publicKey: string
  ): { encryptedData: string; encryptedKey: string; encryptedIV: string } {
