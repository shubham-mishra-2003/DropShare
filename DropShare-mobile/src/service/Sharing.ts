...
//       let retries = 0;
//       let startChunkIndex = 0; // Track where to start sending
//       while (retries < MAX_RETRIES) {
//         try {
//           if (retries > 0) {
//             await new Promise<void>((resolve, reject) => {
...
//               Logger.info(`Sent RESET for ${fileId}`);
//             });
//             startChunkIndex = 0; // Reset to start from beginning
//           }
...
//                   startChunkIndex = chunkIndex + 1; // Resume from next chunk
...
//                 continue; // Skip chunks already sent
//               }
//               const start = chunkIndex * chunkSize;
//               const actualChunkSize = Math.min(chunkSize, fileSize - start);
//               const base64Chunk = await RNFS.read(
...
//             // Store transfer metadata in database
...
//                 // Delete transfer metadata from database
...
//       await RNFS.writeFile(tempPath, "", "base64");
//     } else {
//       // Validate existing transfer metadata
//       if (
