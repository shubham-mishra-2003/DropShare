...
//                 if (parts.length > 2) {
//                   const lastChunkIndex = parseInt(parts[2].split("\n")[0], 10);
//                   if (!isNaN(lastChunkIndex) && lastChunkIndex >= 0) {
//                     startChunkIndex = lastChunkIndex + 1;
//                     transfer.receivedBytes = startChunkIndex * chunkSize;
//                     transfer.progress = (transfer.receivedBytes / fileSize) * 100;
//                     transfer.lastChunkIndex = lastChunkIndex;
//                     fileTransfers.set(fileId, transfer);
//                     Logger.info(`Resuming transfer for ${fileId} from chunk ${startChunkIndex}`);
//                   }
//                 }
//                 resolve();
//               } else {
...
//           for (let batchIndex = Math.floor(startChunkIndex / MAX_CHUNKS_PER_BATCH); batchIndex < totalBatches; batchIndex++) {
...
//             if (batchBuffers.length === 0) continue;
...
//             transfer.receivedBytes = sentBytes;
//             transfer.progress = percentage;
//             transfer.lastChunkIndex = batchStartChunkIndex + batchBuffers.length - 1;
//             fileTransfers.set(fileId, transfer);
...
//               await chunkStorage.updateReceivedChunks(batchData.fileId, chunkData.chunkIndex);
//             }
...
//                 setTransferProgress
//               );
//               const metadata = await chunkStorage.getTransferMetadata(fileId);
//               const lastChunkIndex = metadata && metadata.receivedChunks && metadata.receivedChunks.length > 0
//                 ? Math.max(...metadata.receivedChunks)
//                 : -1;
//               const ackBuffer = Buffer.from(`ACK_FILE:${fileId}:${lastChunkIndex}\n`);
//               socket.write(ackBuffer);
//               const transfer = fileTransfers.get(fileId);
//               if (transfer) {
//                 updateTransferSpeed(
...
//               setTransferProgress
//             );
//             const metadata = await chunkStorage.getTransferMetadata(headerData.fileId);
//             const lastChunkIndex = metadata && metadata.receivedChunks && metadata.receivedChunks.length > 0
//               ? Math.max(...metadata.receivedChunks)
//               : -1;
//             const ackBuffer = Buffer.from(`ACK_FILE:${headerData.fileId}:${lastChunkIndex}\n`);
//             socket.write(ackBuffer);
//             const transfer = fileTransfers.get(headerData.fileId);
//             if (transfer) {
//               updateTransferSpeed(
...
//     const existingMetadata = await chunkStorage.getTransferMetadata(fileId);
//     if (existingMetadata) {
//       if (
//         existingMetadata.fileName !== fileName ||
//         existingMetadata.fileSize !== fileSize ||
//         existingMetadata.totalChunks !== totalChunks ||
//         existingMetadata.chunkSize !== chunkSize
//       ) {
//         Logger.warn(`Metadata mismatch for fileId ${fileId}, resetting`);
//         await chunkStorage.deleteTransfer(fileId);
//         if (await RNFS.exists(tempPath)) {
...
//         await RNFS.writeFile(tempPath, "", "base64");
//       } else {
//         receivedBytes = existingMetadata.receivedChunks.length * chunkSize;
//         lastChunkIndex = existingMetadata.receivedChunks.length > 0
//           ? Math.max(...existingMetadata.receivedChunks)
//           : -1;
//         Logger.info(`Resuming existing transfer for fileId ${fileId} at chunk ${lastChunkIndex}`);
//       }
//     } else {
//       await RNFS.writeFile(tempPath, "", "base64");
//     }
...
//       iv: undefined,
//       status: "Receiving",
//       progress: (receivedBytes / fileSize) * 100,
//       lastChunkIndex,
//       speedWindow: [],
//     };
//     fileTransfers.set(fileId, transfer);

//     // Store or update metadata in database
//     await chunkStorage.storeTransferMetadata({
//       fileId,
//       fileName,
//       fileSize,
//       totalChunks,
//       chunkSize,
//       receivedChunks: existingMetadata?.receivedChunks || [],
...
//   ): Promise<void> {
//     const transfer = fileTransfers.get(fileId);
//     if (!transfer || transfer.isPaused) return;
        ...
        if (receivingFile) {
          ...
          if (dataStr.startsWith("BATCH:")) {
            ...
            if (transfer.receivedBytes === transfer.totalSize) {
              ...
              const finalfileName = `${baseName}_${Date.now()}.${fileExtension}`;
